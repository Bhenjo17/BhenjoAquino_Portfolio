import React, { useState, useEffect } from "react";
  ...
  const App = () => {
    ...
    return (
      // IMPORTANT: Dark class wrapper
      <div className={darkMode ? "dark" : ""}>
        <div className="bg-white dark:bg-slate-950 text-slate-900 dark:text-white min-h-screen transition-colors duration-300">
          
          {/* PRELOADER - Add dark styles */}
          <AnimatePresence>
            {loading && (
              <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-100 flex flex-col items-center justify-center bg-white dark:bg-slate-950">
            ...
            {activeTab === "dashboard" && (
              <div className="overflow-hidden">
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-linear-to-b from-black/20 via-transparent to-slate-50 dark:to-[#0f172a]" />
                  <motion.div 
                  ...
                  >
                    ...
                    <h1 className="text-white text-4xl md:text-6xl font-black mt-4 drop-shadow-2xl">Building Digital <span className="text-black">Experiences</span></h1>
                  </motion.div>
                </motion.div>
  ...
  // --- REUSABLE COMPONENTS (StatCard, ProjectCard, ResumeModal remains same as provided) ---
  const ResumeModal = ({ isOpen, onClose, fileUrl }) => (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-150 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" onClick={onClose}>
          <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 30 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-slate-900 w-full max-w-5xl h-[90vh] rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col">
  ...
  const ProjectCard = ({ project, index }) => (
    <motion.a initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} href={project.link} target={project.link === "#" ? "_self" : "_blank"} rel="noopener noreferrer" className="group bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden hover:shadow-2xl transition-all duration-500 block relative">
