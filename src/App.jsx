...
const App = () => {
  ...
  const skills = [
    ...
    { name: "HTML & CSS", icon: <Layers className="text-orange-600" />, level: "Advanced" },
  ...
  return (
    <div className="flex bg-slate-50 dark:bg-[#0f172a] transition-colors duration-500 min-h-screen overflow-x-hidden font-sans">
      
      {/* PRELOADER */}
      <AnimatePresence>
        {loading && (
          <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white dark:bg-[#0f172a]">
            ...
            <motion.h2 className="mt-6 font-black text-xl tracking-tighter dark:text-white uppercase">BHENJO <span className="text-green-500">AQUINO</span></motion.h2>
          </motion.div>
        )}
      </AnimatePresence>

      <Sidebar darkMode={darkMode} setDarkMode={setDarkMode} activeTab={activeTab} setActiveTab={setActiveTab} />

      <main className={`flex-1 ml-0 md:ml-72 transition-all duration-500 relative ${loading ? 'blur-sm' : 'blur-0'}`}>
        ...
        {activeTab === "dashboard" && (
          <div className="overflow-hidden">
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-slate-50 dark:to-[#0f172a]" />
              <motion.div 
              ...
              >
                ...
                <h1 className="text-white text-4xl md:text-6xl font-black mt-4 drop-shadow-2xl">Building Digital <span className="text-green-400">Experiences</span></h1>
              </motion.div>
            </motion.div>

            <div className="p-6 md:p-12 -mt-10 relative z-10 space-y-32">
              
              {/* CAREER STATUS - SCROLL ANIMATED */}
              <motion.section 
              ...
              >
                <div className="flex items-end justify-between mb-10">
                  ...
                  <div className="hidden md:block h-[2px] flex-1 mx-10 bg-slate-200 dark:bg-slate-800 mb-4" />
              ...
              {/* TECHNICAL ARSENAL - STAGGERED SCROLL ANIMATION */}
              <motion.section 
                className="pb-20"
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {skills.map((skill, i) => (
                    <motion.div 
                      ...
                      className="group p-6 rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-green-500 transition-all duration-300 text-center flex flex-col items-center shadow-sm hover:shadow-green-500/10"
                    >
        ...
        {/* PAGE TRANSITIONS FOR OTHER TABS */}
...
// --- MODAL COMPONENT (REMAINED SAME BUT ADDED SCALE ANIMATION) ---
        ...
        >
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <a href={fileUrl} download className="p-3 bg-green-500 text-white rounded-xl hover:bg-green-600 transition-colors shadow-lg"><Download size={20} /></a>
              <button onClick={onClose} className="p-3 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-white rounded-xl hover:bg-red-500 hover:text-white transition-all"><X size={20} /></button>
            </div>
          </div>
...
const StatCard = ({icon, val, label}) => (
  <motion.div 
    whileHover={{ y: -5 }}
    className="p-6 rounded-[2rem] bg-green-500 text-white flex flex-col justify-center items-center text-center shadow-lg shadow-green-500/20"
  >
    {icon}
...
const ProjectCard = ({ project, index }) => (
  ...
  >
    {project.tag && <span className="absolute top-4 right-4 z-20 bg-green-500 text-white text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">{project.tag}</span>}
    ...
    <div className="p-8">
      ...
      <h3 className="text-xl font-bold mt-2 group-hover:text-green-500 transition-colors dark:text-white leading-tight">{project.title}</h3>
    </div>
  </motion.a>
);
